...
class CitationModel {
  ...
  set authorsAsStrings(authorsAsStrings) {
    ...
    Vue.set(
      this.citation,
      "authors",
      authorsAsStrings.filter(isNonEmptyString).map((name) => ({ name }))
    );
  }
  ...
  get editorsAsStrings() {
    ...
    return this.citation.editors.map((editor) =>
      CitationWrapper.getAgentName(editor)
    );
  }
  ...
  set editorsAsStrings(editors) {
    ...
    Vue.set(
      this.citation,
      "editors",
      editors.filter(isNonEmptyString).map((name) => ({ name }))
    );
  }
  ...
  get seriesEditorsAsStrings() {
    ...
    return this.citation.series_editors.map((editor) =>
      CitationWrapper.getAgentName(editor)
    );
  }
  ...
  set doisAsStrings(dois) {
    this.identifiers = this.identifiers
      ...
      .concat(
        dois.filter(isNonEmptyString).map((doi) => ({ type: "doi", id: doi }))
      );
  }
  ...
  set isbns(isbns) {
    ...
    this.identifiers = this.identifiers
      .filter(
        (identifier) => has(identifier, "type") && identifier.type !== "isbn"
      )
      .concat(
        isbns
          .filter(isNonEmptyString)
          .map((isbn) => ({ type: "isbn", id: isbn }))
      );
  }
  ...
  set issns(issns) {
    ...
    this.identifiers = this.identifiers
      .filter(
        (identifier) => has(identifier, "type") && identifier.type !== "issn"
      )
      .concat(
        issns
          .filter(isNonEmptyString)
          .map((issn) => ({ type: "issn", id: issn }))
      );
  }
  ...
  get urlsAsStrings() {
    ...
    if (has(this.citation, "link"))
      return this.citation.link.map((link) => link.url);
    return [];
  }
  ...
  set urlsAsStrings(urls) {
    ...
    Vue.set(
      this.citation,
      "link",
      urls.filter(isNonEmptyString).map((url) => ({ url }))
    );
  }
  ...
  get url() {
    ...
    if (this.isbns.length > 0)
      return `https://www.worldcat.org/search?q=bn%3A${encodeURI(
        this.isbns[0]
      )}`;
    if (this.issns.length > 0)
      return `https://www.worldcat.org/search?q=n2%3A${encodeURI(
        this.issns[0]
      )}`;
    ...
    if (has(this.citation, "type") && has(this.citation, "title")) {
      ...
      if (this.citation.type === "article")
        return `https://search.crossref.org/?q=${encodeURI(
          this.citation.title
        )}`;

      // We should search book and book sections against WorldCat.
      return `https://www.worldcat.org/search?q=${encodeURI(
        this.citation.title
      )}`;
    }
...
export default {
  ...
  mutations: {
    ...
    setCitations(state, payload) {
      Vue.set(payload.object, payload.citationKey, payload.citations);
    },
  },
};
