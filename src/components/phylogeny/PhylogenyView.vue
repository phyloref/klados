<template>
  <div>
    ...
    <div class="card mt-2">
      <h5 class="card-header">Taxonomic units in this phylogeny</h5>
      <div class="card-body">
        To add additional taxonomic units to this list, please label the
        corresponding node on the phylogeny.
      </div>
      ...
      >
        ...
        <template #cell(additional_taxonomic_units)="row">
          {{ row.item.additional_taxonomic_units }} taxonomic units
          <b-button
            variant="primary"
            @click="addTUnitForNodeLabel(row.item.node_label)"
            class="float-right"
            size="sm"
            >Add</b-button
          >
        ...
        <template #row-details="row">
          <b-card>
            <b-row
              v-for="(tunit, index) in getExplicitTUnitsForLabel(
                row.item.node_label
              )"
            ...
            >
              <Specifier
                ...
                :remote-specifier-id="
                  'tunit_' + row.item.node_label + '_' + index
                "
              />
            </b-row>
          </b-card>
...
export default {
  ...
  computed: {
    ...
    terminalLabelsSorted() {
      ...
      return new PhylogenyWrapper(this.selectedPhylogeny)
        .getNodeLabels("terminal")
        .sort();
    },
    internalLabelsSorted() {
      // Return a list of internal (i.e. non-leaf node) labels sorted alphabetically.
      return new PhylogenyWrapper(this.selectedPhylogeny)
        .getNodeLabels("internal")
        .sort();
    },
    taxonomicUnitsTable() {
